...
# Путь к шаблону PDF — относительно проекта, не зависит от CWD
_HERE = Path(__file__).resolve()
_TEMPLATE_PDF = None
for _candidate in [
    _HERE.parents[1] / "templates" / "kp_pdf.html",  # backend/templates/
    _HERE.parents[2] / "templates" / "kp_pdf.html",  # <root>/templates/
]:
    if _candidate.exists():
        _TEMPLATE_PDF = _candidate
        break
...
class GigaChatService:
    ...
    def _md_table_to_html(kp_text: str) -> str:
        ...
        for i, row in enumerate(rows):
            ...
            style_cell = "background:#0a1f44;color:#fff;font-weight:700;" if i == 0 else ""
            html.append("<tr>")
            for cell in row:
    ...
    def _build_prompt(self, context: Dict) -> str:
        ...
        # Таблица доп. услуг прямо в промпте — GigaChat копирует её 1:1
        services_lines = ""
...
Важно:
- Должна быть таблица с точными цифрами выше (Base / Скидка / После скидки / Услуги / ИТОГО).
- Упомяни выбранные услуги ({services_names}) и тип оплаты ({ptype}).
- Если ипотека — укажи, что скидка 0%, но фиксируется базовая цена.
    ...
    def generate_kp_text(self, context: Dict) -> Dict:
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {"role": "system", "content": "Ты — аналитик недвижимости. Отвечай только в JSON."},
...
Мы ценим наше партнерство. Скидка {{ discount_percent }}% фиксируется для типа оплаты {{ payment_type }}. При полной оплате скидка составляет {{ discount_amount_fmt }} рублей.
...
Срок действия КП: 3 дня с момента формирования."""
    ...
    def generate_pdf(self, context: Dict, output_path: str = "/tmp/kp_dsk.pdf") -> str:
        if _TEMPLATE_PDF is None or not Path(str(_TEMPLATE_PDF)).exists():
            raise FileNotFoundError(
                f"kp_pdf.html не найден. Искали рядом с {_HERE}. "
                f"Проверьте backend/templates/kp_pdf.html"
            )
        ...
        final_num = int(round(float(kp_data.get("final_price") or gc.get("final_price") or 0)))
        base_num = int(round(float(kp_data.get("base_price") or gc.get("base_price") or 0)))
    ...
    def analyze_dialog(self, dialog_text: str) -> List[Dict]:
        ...
        prompt = f"""
        Проанализируй диалог. Выяви ВСЕ возражения. Верни JSON-массив объектов:
        - "objection_type": строка
        - "trigger_word": ключевое слово
        - "response_template": готовый ответ
        - "conversion_tip": совет
        Диалог: \"{dialog_text}\"
        ...
        try:
            ...
            payload = {
                "model": settings.GIGACHAT_MODEL,
                "messages": [
                    {"role": "system", "content": "Аналитик продаж. Отвечай только JSON."},
                    {"role": "user", "content": prompt},
                ],
                "temperature": 0.3,
                "max_tokens": 1000,
            ...
            logger.error(f"Ошибка analyze_dialog: {e}")
            return []
